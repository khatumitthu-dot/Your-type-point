...
async function api(req,res,p){
 ...
 try{
  ...
  if(req.method==='POST'&&p==='/api/admin/upload'){
   ...
   audit('upload',{filename,type:m[1],size:buf.length});save(db);
   return send(res,201,{ok:true,filename,url:'/uploads/'+encodeURIComponent(filename),type:m[1],size:buf.length},'application/json',origin);
  }
  ...
  if(req.method==='PATCH'&&p==='/api/admin/store-settings'){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    const x=await body(req);db.site.store={...(db.site.store||{}),name:String(x.name||'YOUR TYPE'),phone:String(x.phone||''),whatsapp:String(x.whatsapp||''),email:String(x.email||''),address:String(x.address||''),currency:String(x.currency||'INR')};audit('store.settings.update');save(db);return send(res,200,{ok:true,store:db.site.store},'application/json',origin);
  }
  if(req.method==='PATCH'&&p==='/api/admin/site-content'){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    const x=await body(req);db.site.content={...(db.site.content||{}),banner:String(x.banner||''),bannerButton:String(x.bannerButton||''),bannerLink:String(x.bannerLink||'')};audit('site.content.update');save(db);return send(res,200,{ok:true,content:db.site.content},'application/json',origin);
  }
  ...
  if(req.method==='DELETE'&&p.startsWith('/api/admin/media/')){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    const name=path.basename(decodeURIComponent(p.slice('/api/admin/media/'.length))),file=path.join(UPLOADS,name);if(!fs.existsSync(file)||!fs.statSync(file).isFile())return send(res,404,{error:'Media not found'},'application/json',origin);fs.unlinkSync(file);audit('media.delete',{name});save(db);return send(res,200,{ok:true},'application/json',origin);
  }
  ...
  if(req.method==='POST'&&p==='/api/admin/notifications/test'){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    audit('notification.test');save(db);return send(res,200,{ok:true,message:'Notification test recorded. Configure a real provider/webhook to deliver externally.'},'application/json',origin);
  }
  if(req.method==='POST'&&p==='/api/admin/reset-role'){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    const x=await body(req);if(!verifyAdminPassword(String(x.currentPassword||'')))return send(res,401,{error:'Current password is incorrect'},'application/json',origin);db.settings.role='Super Admin';audit('admin.role.reset');save(db);return send(res,200,{ok:true,role:'Super Admin'},'application/json',origin);
  }
...
server.listen(PORT,'0.0.0.0',()=>console.log(`YOUR TYPE running at http://localhost:${PORT}`));
