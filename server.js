...
let supabaseProductsLoading=null;
...
async function supabaseRequest(pathname,options={}){
  if(!SUPABASE_ENABLED)throw new Error('Supabase is not configured. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.');
  const r=await fetch(SUPABASE_URL+'/rest/v1/'+pathname,{...options,headers:supabaseHeaders(options.headers||{})});
  const text=await r.text();
  let data=null;try{data=text?JSON.parse(text):null}catch{data=text}
  if(!r.ok)throw new Error('Supabase '+r.status+': '+(data?.message||data?.hint||data?.error||text||'request failed'));
  return data;
}
function productToSupabase(p){return {id:String(p.id),name:String(p.name||''),price:String(p.price||''),image:String(p.image||''),images:Array.isArray(p.images)?p.images:[],badge:String(p.badge||''),old_price:String(p.oldPrice||''),description:String(p.description||''),category:String(p.category||'T-Shirts'),sku:String(p.sku||''),cost:Number(p.cost||0),sizes:p.sizes||cleanSizes(),colors:Array.isArray(p.colors)?p.colors:[],active:p.active!==false,featured:Boolean(p.featured),sections:Array.isArray(p.sections)?p.sections:[]};}
function productFromSupabase(p){return safeProduct({id:p.id,name:p.name,price:p.price,image:p.image,images:Array.isArray(p.images)?p.images:[],badge:p.badge,oldPrice:p.old_price,description:p.description,category:p.category,sku:p.sku,cost:p.cost,sizes:p.sizes,colors:p.colors,active:p.active,featured:p.featured,sections:p.sections});}
async function loadProductsFromSupabase(){
  if(!SUPABASE_ENABLED||supabaseProductsLoaded)return;
  if(supabaseProductsLoading)return supabaseProductsLoading;
  supabaseProductsLoading=(async()=>{
    const rows=await supabaseRequest('products?select=*&order=created_at.asc');
    db.products=Array.isArray(rows)?rows.map(productFromSupabase):[];
    supabaseProductsLoaded=true;
    save(db);
  })().catch(e=>{console.error('[Supabase] products load failed:',e.message);throw e}).finally(()=>{supabaseProductsLoading=null});
  return supabaseProductsLoading;
}
async function upsertSupabaseProduct(p){if(SUPABASE_ENABLED)await supabaseRequest('products?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify(productToSupabase(p))});}
async function deleteSupabaseProduct(id){if(SUPABASE_ENABLED)await supabaseRequest('products?id=eq.'+encodeURIComponent(String(id)),{method:'DELETE',headers:{Prefer:'return=minimal'}});}
async function syncAllProductsToSupabase(){if(SUPABASE_ENABLED&&db.products.length)await supabaseRequest('products?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(db.products.map(productToSupabase))});}
...
function normalizeImageRef(raw){const v=String(raw||'').trim();if(!v)return '';if(/^data:|^https?:|^\//i.test(v))return v;if(/^photos\//i.test(v))return '/'+v;if(/^uploads\//i.test(v))return '/'+v;return '/uploads/'+encodeURIComponent(v.replace(/^\.\//,''))}
...
async function api(req,res,p){
 ...
 if(SUPABASE_ENABLED && (p==='/api/products'||p.startsWith('/api/admin/products'))) { try { await loadProductsFromSupabase(); } catch(e) { return send(res,502,{error:'Database connection failed',detail:e.message},'application/json',origin); } }
 try{
  ...
  if(req.method==='DELETE'&&p.startsWith('/api/admin/orders/')){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const id=decodeURIComponent(p.slice('/api/admin/orders/'.length)),i=db.orders.findIndex(v=>v.orderId===id);if(i<0)return send(res,404,{error:'Order not found'},'application/json',origin);db.orders.splice(i,1);audit('order.delete',{orderId:id});save(db);return send(res,200,{ok:true},'application/json',origin)}

  if(req.method==='PATCH'&&p.startsWith('/api/admin/reviews/')){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const id=decodeURIComponent(p.slice('/api/admin/reviews/'.length)),x=await body(req),r=db.reviews.find(v=>v.id===id);if(!r)return send(res,404,{error:'Review not found'},'application/json',origin);if(x.status!==undefined&&['pending','approved','rejected'].includes(x.status))r.status=x.status;if(x.reply!==undefined)r.reply=String(x.reply);save(db);return send(res,200,r,'application/json',origin)}
  if(req.method==='DELETE'&&p.startsWith('/api/admin/reviews/')){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const id=decodeURIComponent(p.slice('/api/admin/reviews/'.length)),n=db.reviews.length;db.reviews=db.reviews.filter(v=>v.id!==id);if(n===db.reviews.length)return send(res,404,{error:'Review not found'},'application/json',origin);save(db);return send(res,200,{ok:true},'application/json',origin)}
  ...
  if(req.method==='POST'&&p==='/api/admin/products/bulk'){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const x=await body(req),ids=Array.isArray(x.ids)?x.ids.map(String):[],action=String(x.action||'');if(!ids.length||!['show','hide','delete'].includes(action))return send(res,400,{error:'Invalid bulk action'},'application/json',origin);if(action==='delete'){db.deletedProductIds=[...new Set(db.deletedProductIds.concat(ids))];ids.forEach(removeProductFromSections);db.products=db.products.filter(p=>!ids.includes(String(p.id)));}else db.products.forEach(p=>{if(ids.includes(String(p.id))&&!productDeleted(p.id))p.active=action==='show'});audit('products.bulk',{action,ids});save(db);await syncAllProductsToSupabase();return send(res,200,{ok:true},'application/json',origin)}
  if(req.method==='POST'&&p==='/api/admin/upload'){
   if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const x=await body(req),name=path.basename(String(x.name||'')).replace(/[^a-zA-Z0-9._-]/g,'_'),data=String(x.data||''),m=data.match(/^data:(image\/(?:png|jpeg|webp));base64,(.+)$/);if(!name||!m)return send(res,400,{error:'Valid PNG, JPG or WEBP image data is required'},'application/json',origin);const buf=Buffer.from(m[2],'base64');if(buf.length>5*1024*1024)return send(res,413,{error:'Image must be 5MB or smaller'},'application/json',origin);const ext=m[1]==='image/jpeg'?'.jpg':m[1]==='image/webp'?'.webp':'.png',base=path.basename(name,path.extname(name))||'image',filename=base.replace(/[^a-zA-Z0-9._-]/g,'_')+'-'+crypto.randomBytes(4).toString('hex')+ext;fs.writeFileSync(path.join(UPLOADS,filename),buf);audit('upload',{filename});save(db);return send(res,201,{ok:true,filename,url:'/uploads/'+encodeURIComponent(filename)},'application/json',origin);
  }
  ...
  if(req.method==='POST'&&p==='/api/admin/products'){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const x=await body(req);const image=normalizeImageRef(x.image||((x.images||[])[0]||''));if(!String(x.name||'').trim()||priceNumber(x.price)<=0||!image)return send(res,400,{error:'Name, price and image are required'},'application/json',origin);const pr=safeProduct({...x,id:'p_'+crypto.randomBytes(6).toString('hex'),image,sizes:cleanSizes(x.sizes)});db.products.push(pr);audit('product.create',{id:pr.id});save(db);await upsertSupabaseProduct(pr);return send(res,201,pr,'application/json',origin)}
  if(req.method==='PATCH'&&p.startsWith('/api/admin/products/')){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const id=decodeURIComponent(p.slice('/api/admin/products/'.length)),x=await body(req),i=db.products.findIndex(v=>String(v.id)===id);if(productDeleted(id))return send(res,410,{error:'Product permanently deleted'},'application/json',origin);if(i<0)return send(res,404,{error:'Product not found'},'application/json',origin);const current=db.products[i],merged={...current,...x,id:current.id,image:normalizeImageRef(x.image||((Array.isArray(x.images)&&x.images[0])||current.image)),sizes:x.sizes?cleanSizes(x.sizes):cleanSizes(current.sizes)};if(!merged.name||priceNumber(merged.price)<=0||!merged.image)return send(res,400,{error:'Name, price and image are required'},'application/json',origin);db.products[i]=safeProduct(merged);audit('product.update',{id});save(db);await upsertSupabaseProduct(db.products[i]);return send(res,200,db.products[i],'application/json',origin)}
  if(req.method==='DELETE'&&p.startsWith('/api/admin/products/')){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const id=decodeURIComponent(p.slice('/api/admin/products/'.length)),i=db.products.findIndex(v=>String(v.id)===id);if(i<0&&productDeleted(id))return send(res,200,{ok:true,alreadyDeleted:true},'application/json',origin);if(i<0)return send(res,404,{error:'Product not found'},'application/json',origin);db.products.splice(i,1);db.deletedProductIds=[...new Set(db.deletedProductIds.concat(id))];removeProductFromSections(id);audit('product.delete',{id,permanent:true});save(db);await deleteSupabaseProduct(id);return send(res,200,{ok:true,permanent:true},'application/json',origin)}
  ...
  if(req.method==='DELETE'&&p.startsWith('/api/admin/coupons/')){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const id=decodeURIComponent(p.slice('/api/admin/coupons/'.length)),n=db.coupons.length;db.coupons=db.coupons.filter(v=>v.id!==id);if(n===db.coupons.length)return send(res,404,{error:'Coupon not found'},'application/json',origin);save(db);return send(res,200,{ok:true},'application/json',origin)}
